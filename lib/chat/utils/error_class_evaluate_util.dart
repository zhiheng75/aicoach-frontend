...
class ErrorClassEvaluateUtil {
  ErrorClassEvaluateUtil();

  WebSocketChannel? _websocket;
  final String _oss = 'evaluate_oss';

  void evaluate(ErrorClassMessageEntity message,
      Function(Map<String, dynamic> params) onSuccess) {
    String pathName = '${getDate()}/${getRandom(12)}.${message.sessionId}';
...
//    right = right + (min + (Random().nextInt(max - min))).toString();
  ...
  Future<void> _evaluate({
    required ErrorClassMessageEntity message,
  ...
  Map<String, dynamic> _getParams(
      ErrorClassMessageEntity message, Map<String, dynamic> evaluation) {
    return {
      'mistake_id': message.sessionId,
      'suggestion_sentence': message.tesuggestionSentencext,
      'suggestion_audio': message.suggestionAudio,
      'user_practice_sentence': message.text,
      'user_practice_audio': message.speechfile,
      'user_practice_score': evaluation['total_score'],
    };
  }
