...
class _HomePageState extends State<HomePage>
  ...
  void getDefaultTeacher() async {
    HomeTeacherProvider provider =
        Provider.of<HomeTeacherProvider>(context, listen: false);
    String userId = await provider.getUserId();
    Map<dynamic, dynamic>? value =
        SpUtil.getObject('${Constant.teacher}_$userId');
    if (value != null) {
      TeachListEntity teacher =
          TeachListEntity.fromJson(value as Map<String, dynamic>);
                    ...
                    child: Container(
                      ...
                      child: GestureDetector(
                          onTap: () async {
                            ...
                            if (provider.isVip == 0 &&
                                (!provider.hasFreeUsage ||
                                    provider.availableTime == 0)) {
                              closeProgress();
  ...
  void _showTimeOutBottomSheet() {
    showModalBottomSheet<void>(
      ...
      isDismissible: false,
      builder: (_) =>
          Consumer<ConversationProvider>(builder: (_, provider, __) {
        Widget title = Padding(
          padding: const EdgeInsets.only(left: 20, right: 20, top: 50),
          child: Text(
            provider.isVip == 0
                ? "体验到期, 与你的专属AI外教开启学习之旅"
                : (provider.isVip == 2 ? "会员到期，邀请您继续一路同行" : "您的时长已用完"),
            style: TextStyle(
                color: Colours.color_111B44, fontSize: Dimens.font_sp22),
          ),
        );
